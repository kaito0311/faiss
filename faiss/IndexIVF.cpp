...
void IndexIVF::add_sa_codes(idx_t n, const uint8_t* codes, const uint8_t* qualities, const idx_t* xids) {
    FAISS_THROW_MSG("IndexIVF::add_sa_codes with quality is not implemented");
}
...
void IndexIVF::add_core(
    ...
    encode_vectors(n, x, coarse_idx, flat_codes.get()); // move data from x to flat codes, can encode list_no to if needed
...
void IndexIVF::encode_vectors_with_quality(
    ...
    FAISS_THROW_MSG("IndexIVF::encode_vectors with quality is not implemented");
}
...
void IndexIVF::search_with_quality(
        idx_t n, 
        const float* x, 
        idx_t k, 
...
#pragma omp parallel for if (nt > 1)
        ...
        // handle parallelization at level below (or don't run in parallel at
        // all)
        sub_search_func(n, x, lower_quality, upper_quality, distances, labels, out_quas, &indexIVF_stats);
    }
...
void IndexIVF::search_preassigned_with_quality(
        idx_t n,
        const float* x,
        idx_t k,
        const idx_t* keys,
    ...
    FAISS_THROW_IF_NOT_MSG(invlists->has_quality(0), "Quality array is empty");
...
#pragma omp parallel if (do_parallel) reduction(+ : nlistv, ndis, nheap)
    {
        ...
        auto init_result = [&](float* simi, idx_t* idxi, float* quai) {
            if (!do_heap_init)
                return;
            if (metric_type == METRIC_INNER_PRODUCT) {
                heap_heapify_quality<HeapForIP>(k, simi, idxi, quai);
            } else {
                heap_heapify_quality<HeapForL2>(k, simi, idxi, quai);
            }
        };

        auto add_local_results = [&](const float* local_dis,
                                     const idx_t* local_idx,
                                     const float* local_qua,
                                     float* simi,
                                     idx_t* idxi,
                                     float* quai) {
            if (metric_type == METRIC_INNER_PRODUCT) {
                heap_addn_quality<HeapForIP>(k, simi, idxi, quai, local_dis, local_idx, local_qua, k);
            } else {
                heap_addn_quality<HeapForL2>(k, simi, idxi, quai, local_dis, local_idx, local_qua, k);
            }
        };

        auto reorder_result = [&](float* simi, idx_t* idxi, float* quai) {
            if (!do_heap_init)
                return;
            if (metric_type == METRIC_INNER_PRODUCT) {
                heap_reorder_quality<HeapForIP>(k, simi, idxi, quai);
            } else {
                heap_reorder_quality<HeapForL2>(k, simi, idxi, quai);
            }
        };
        ...
        auto scan_one_list = [&](idx_t key, // id of cluster
            ...
            try {
                if (invlists->use_iterator) {
                    ...
                    nheap += scanner->iterate_codes_with_quality(
                            it.get(), simi, idxi, quai, k, list_size);
                    ...
                    InvertedLists::ScopedQualities squalities(invlists, key); // take entire a cluster quality
                    const uint8_t* qualities = squalities.get();
                    ...
                    if (selr) { // IDSelectorRange
                        ...
                        codes += jmin * code_size;
                        ids += jmin;
                        qualities += jmin * qua_size; 
                    }

                    nheap += scanner->scan_codes_with_quality(
                            list_size, codes, ids, qualities, lower_quality, upper_quality, simi, idxi, quai, k);
                    // nheap += scanner->scan_codes(
                    //         list_size, codes, ids, simi, idxi, k);
                    return list_size;
                }
...
#pragma omp for
            for (idx_t i = 0; i < n; i++) {
                ...
                float* simi = distances + i * k;
                idx_t* idxi = labels + i * k;
                float* quai = out_quas + i * k;

                init_result(simi, idxi, quai);
            ...
            std::vector<idx_t> local_idx(k);
            std::vector<float> local_dis(k);
            std::vector<float> local_qua(k);

            for (size_t i = 0; i < n; i++) {
                scanner->set_query(x + i * d);
                init_result(local_dis.data(), local_idx.data(), local_qua.data());
...
#pragma omp critical
                {
                    add_local_results(
                            local_dis.data(), local_idx.data(), local_qua.data(), simi, idxi, quai);
                }
#pragma omp barrier
#pragma omp single
                reorder_result(simi, idxi, quai);
...
#pragma omp single
            for (int64_t i = 0; i < n; i++) {
                init_result(distances + i * k, labels + i * k, out_quas + i * k);
            }
...
#pragma omp single
            for (int64_t i = 0; i < n; i++) {
                reorder_result(distances + i * k, labels + i * k, out_quas + i * k);
            }
        } else {
            FAISS_THROW_FMT("parallel_mode %d not supported\n", pmode);
        }
...
void IndexIVF::boundary_search_preassigned_with_quality(
        idx_t n,
        const float* x,
        idx_t k,
        const float lower,
        ...
        auto scan_one_list = [&](const float lower, 
            ...
            try {
                if (invlists->use_iterator) {
                    ...
                    nheap += scanner->iterate_codes_boundary_with_quality(
                            it.get(), lower, upper, duplicate_thr, rm_duplicate, simi, idxi, quai, k, list_size);
                    ...
                    nheap += scanner->scan_codes_boundary_with_quality(
                            list_size, lower, upper, duplicate_thr, rm_duplicate, codes, ids, qualities, lower_quality, upper_quality, simi, idxi, quai, k);
                    // nheap += scanner->scan_codes(
                    //         list_size, codes, ids, simi, idxi, k);
                    return list_size;
                }
...
void IndexIVF::boundary_search_with_quality(
        idx_t n, 
        const float* x, 
        idx_t k, 
        const float lower,
    ...
    auto sub_search_func = [this, k, nprobe, params](
        ...
        invlists->prefetch_lists(idx.get(), n * nprobe);
        // printf("Search search_preassigned_with_quality \n");
        boundary_search_preassigned_with_quality(
                n,
                x,
                k,
                lower,
...
#pragma omp parallel for if (nt > 1)
        ...
        // handle parallelization at level below (or don't run in parallel at
        // all)
        sub_search_func(n, x, lower, upper, duplicate_thr, rm_duplicate, lower_quality, upper_quality, distances, labels, out_quas, &indexIVF_stats);
    }
...
void IndexIVF::reconstruct_qua(idx_t key, float* qua_recons) const {
    idx_t lo = direct_map.get(key);
    reconstruct_qua_from_offset(lo_listno(lo), lo_offset(lo), qua_recons);
}
...
void IndexIVF::reconstruct_qua_n(idx_t i0, idx_t ni, float* qua_recons) const {
    FAISS_THROW_IF_NOT(ni == 0 || (i0 >= 0 && i0 + ni <= ntotal));
    FAISS_THROW_IF_NOT(invlists->has_quality(0));

    for (idx_t list_no = 0; list_no < nlist; list_no++) {
        size_t list_size = invlists->quality_list_size(list_no);
        ScopedIds idlist(invlists, list_no);
        ...
        for (idx_t offset = 0; offset < list_size; offset++) {
            ...
            float* reconstructed = qua_recons + (id - i0) * d;
            reconstruct_qua_from_offset(list_no, offset, reconstructed);
        }
    }
}
...
/* standalone codec interface */
size_t IndexIVF::sa_qua_code_size() const {
    size_t coarse_size = coarse_code_size();
    return qua_size + coarse_size;
}
...
void IndexIVF::sa_qua_encode(idx_t n, const float* r_qua, uint8_t* bytes) const {
    FAISS_THROW_MSG("IndexIVF::sa_qua_encode is not implemented");
}
    ...
    if (!keep_max) {
        ...
        for (size_t j = 0; j < list_size; j++) {
            ...
            if (curr_quality >= lower_quality && curr_quality <= upper_quality) {
                ...
                if (dis > simi[0]) {
                    int64_t id = store_pairs ? lo_build(list_no, j) : ids[j];
                    minheap_replace_top_quality(k, simi, idxi, quai, dis, id, curr_quality);
                    nup++;
            }
            }
            codes += code_size;
...
size_t InvertedListScanner::scan_codes_boundary_with_quality(
        size_t list_size, 
        const float lower,
        const float upper,
        const float duplicate_thr,
...
size_t InvertedListScanner::iterate_codes_boundary_with_quality(
        InvertedListsIterator* it,
        const float lower,
        const float upper,
        const float duplicate_thr,
        ...
        size_t k,
        size_t& list_size) const {
    FAISS_THROW_MSG("InvertedListScanner::iterate_codes_boundary_with_quality not implemented");
}

size_t InvertedListScanner::iterate_codes_with_quality(
        InvertedListsIterator* it,
        float* simi,
        idx_t* idxi,
        float* quai,
        size_t k,
        size_t& list_size) const {
    FAISS_THROW_MSG("InvertedListScanner::iterate_codes_with_quality not implemented");
}
